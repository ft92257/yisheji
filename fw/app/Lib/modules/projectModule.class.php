  ...
  public function submit_deal()
    ...
    $json_arr = array(
      ...
      'jump' => '',
		...
		if($deal_item)
		{
      if ($deal_item['is_edit']) {
        $json_arr['info'] = '项目审核中，请勿重复提交！';
        ECHO json_encode($json_arr);
      }
      else {
        $GLOBALS['db']->query("update ".DB_PREFIX."deal set is_edit = 1 where id = ".$id);
        $json_arr['status'] = TRUE;
        $json_arr['info'] = '提交成功，等待管理员审核！';
        ECHO json_encode($json_arr);
      }
		}
		else
		{
      $json_arr['info'] = '提交失败，请重新操作！';
			ECHO json_encode($json_arr);
		}
	}
