...
class dealsModule extends BaseModule
{
	...
	{		
		...
		//获得分类名
		$p_r_name = $GLOBALS['db']->getOne("select name from ".DB_PREFIX."deal_cate where id=".$id." order by id");
		$GLOBALS['tmpl']->assign("p_id",$id);
		$GLOBALS['tmpl']->assign("p_id_name",$p_r_name);
    ...
    $loc_result = load_dynamic_cache("INDEX_CATE_LOC");
		if($loc_result===false)
		{
			...
			set_dynamic_cache("INDEX_CATE_LIST",$loc_result);
		}
		$GLOBALS['tmpl']->assign("loc_list",$loc_result);
