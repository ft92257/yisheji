...
<DIV>
  ...
  height=47>
      ...
      style="PADDING-RIGHT: 10px; PADDING-LEFT: 10px; FLOAT: left; BACKGROUND-IMAGE: url(__STATICS__/images/main_hb.gif); PADDING-BOTTOM: 10px; COLOR: white; PADDING-TOP: 10px; BACKGROUND-REPEAT: repeat-x; HEIGHT: 47px; TEXT-ALIGN: center; 0px: ">评论管理 </SPAN><SPAN 
    ...
    vAlign=top align=middle>
      ...
      border=1><form method = "post" >
					<tr><td>评论开始时间:</td><td ><input type = "text" name="stime" onfocus="HS_setDate(this)"/></td></tr>
					<tr><td>评论结束时间：</td><td><input type = "text" name = "etime" onfocus="HS_setDate(this)"/></td></tr>
					<tr><td>按帐户查询: </td><td ><input type = "text" name = "username"/></td></tr>
					<tr><td>按内容查询:</td><td ><input type = "text" name = "comment"/></td></tr>
                ...
                <TR>
                  <td class=gridViewHeader style="WIDTH: 85px" scope=col>&nbsp;</TH>
                  <th class=gridViewHeader scope=col style="width:155px">评论Id</td>
                  <th class=gridViewHeader scope=col>用户名</td>
                  <th class=gridViewHeader scope=col>评论类型</td>
                  <th class=gridViewHeader scope=col>用户id</td>
                  <th class=gridViewHeader scope=col>评分</td>
                  <th class=gridViewHeader scope=col>评论内容</td> 
                  <th class=gridViewHeader scope=col>评论时间</td>       
                </TR>
                <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
					  	 <TD class=gridViewItem style="WIDTH: 80px"><IMG src="__STATICS__/index/EmployeeMgr/bg_users.gif"></TD>
					     <td class=gridViewItem><?php echo ($vo["id"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["username"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["type"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["uid"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["score"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["comment"]); ?></td>
