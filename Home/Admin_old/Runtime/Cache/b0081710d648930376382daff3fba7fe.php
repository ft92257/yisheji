  ...
  height=47>
    ...
    vAlign=top align=middle>
      ...
      border=1><form method = "post" >
															<tr><td>装修类型：</td><td ><select name="decoration" style="width:80px;height:30px;border:none">
											                      <option value="0">徐汇区</option>
											                      ...
											                      <option value="6" selected>请选择</option>
											      </select></td></tr>
															<tr><td>装修风格：</td><td><select name="style" style="width:80px;height:30px;border:none">
										                      <option value="xhq">徐汇区</option>
										                      <option value="ptq">普陀区</option>
										                      <option value="hpq">黄浦区</option>
										                      <option value="jaq">静安区</option>
										                      <option value="cnq">长宁区</option>
                ...
                <TR>
                  <th class=gridViewHeader style="WIDTH: 70px" scope=col>&nbsp;</TH>
                  <th class=gridViewHeader scope=col style="WIDTH: 80px">案例Id</td>
                  <th class=gridViewHeader scope=col>案列名称</td> 
                  <th class=gridViewHeader scope=col>装修类型</td>
                  <th class=gridViewHeader scope=col>装修风格</td>
                  <th class=gridViewHeader scope=col>收藏数量</td>
                  <th class=gridViewHeader scope=col>审核案例</td>
                ...
                <?php if(is_array($data)): foreach($data as $key=>$vo): ?><tr>
					     ...
					     <td class=gridViewItem><?php echo ($vo["decoration_type"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["style"]); ?></td>
					     <td class=gridViewItem><?php echo ($vo["collect_count"]); ?></td>
					      <?php if($vo["status"] == 0): ?><td class=gridViewItem><a href="<?php echo U('/Case/update/id');?>/<?php echo ($vo["id"]); ?>" style="color:red">取消通过</a></td><?php else: ?>
     					 <td class=gridViewItem><a href="<?php echo U('/Case/update/id');?>/<?php echo ($vo["id"]); ?>">通过审核</a></td><?php endif; ?>
